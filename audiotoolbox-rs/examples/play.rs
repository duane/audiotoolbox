...
pub unsafe extern "C" fn noop_output_callback(in_user_data: *mut c_void, in_aq: AudioQueueRef, in_buf: AudioQueueBufferRef) {
  ...
  let mut packet_descriptions: &mut Option<Vec<AudioStreamPacketDescription>> = &mut (*player).packet_descriptions;
  let mut status = AudioFileReadPackets(
    (*player).playback_file.as_ref(),
    false as u8,
    &mut num_bytes,
    (*packet_descriptions).as_ref().map(|v| v.as_ptr()).unwrap_or(ptr::null_mut()) as *mut AudioStreamPacketDescription,
  ...
  if n_packets > 0 {
    status = AudioQueueEnqueueBuffer(
      in_aq,
      in_buf,
      (*packet_descriptions).as_ref().map(|v|v.len() as u32).unwrap_or(0),
      (*packet_descriptions).as_ref().map(|v|v.as_ptr()).unwrap_or(ptr::null_mut())
    );
    if status != 0 {
      panic!("Got bad status: {:?}", status);
    }
    ...
    (*player).is_done = true;
    AudioQueue(in_aq).stop(false);
  }
}

fn bytes_for_time(audio_file: &mut AudioFile, desc: AudioStreamBasicDescription, seconds: f64) -> Result<(u32, u32), OSStatus> {
  let max_packet_size = audio_file.get_packet_size_upper_bound()?;
...
fn main() {
  ...
  let mut audio_file = AudioFile::open(file_url).unwrap();
  let data_format = audio_file.get_data_format().unwrap();
  ...
  let mut audio_queue = AudioQueue::new_output(Some(noop_output_callback), &mut player as *mut Player as *mut c_void, &data_format).unwrap();
  let (buffer_byte_size, num_packets_to_read) = bytes_for_time(&mut player.playback_file, data_format, 0.5).unwrap();
  ...
  if is_format_vbr {
    ...
    for p in 0..num_packets_to_read {
  ...
  audio_queue.copy_cookie_to_queue(&mut player.playback_file);
  let playback_buffer_count = 3;
  let mut buffers: Vec<Buffer> = Vec::with_capacity(playback_buffer_count);
  for i in 0..playback_buffer_count {
    let mut buf = Buffer::new(&mut audio_queue, buffer_byte_size).unwrap();
    unsafe {
      noop_output_callback(&mut player as *mut Player as *mut c_void, audio_queue.as_ref(), buf.as_ref());
    }
    buffers.push(buf);
  }
